...
// ── ReporteModal — 100% interno, sin window.open ni popups ───────────────────
function ReporteModal({ data, fmt, MESES, rango, setRango, onClose }) {
 ...
 onClick={e=>e.target===e.currentTarget&&onClose()}>
 <div className="fade-in" style={{background:"#080808",border:"1px solid #a78bfa30",borderRadius:8,width:"100%",maxWidth:vista&&res?880:460,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 0 60px #a78bfa20",transition:"max-width .3s"}}>
 {/* Header */}
 ...
 <div style={{fontSize:10,color:"#444",marginTop:3,letterSpacing:1}}>Selecciona el rango de fechas a analizar</div>
 </div>
 <button onClick={onClose} style={{background:"transparent",color:"#444",border:"1px solid #222",padding:"8px 16px",borderRadius:4,cursor:"pointer",fontFamily:"inherit",fontSize:12,letterSpacing:1}}>CANCELAR</button>
 </div>
 ...
 <div>
 <div style={{fontFamily:"Orbitron,monospace",fontSize:16,fontWeight:900,color:"#00d4ff",letterSpacing:3}}>NEXU<span style={{color:"#ff6b35"}}>STORE</span><span style={{color:"#ff6b35"}}> RD</span></div>
 <div style={{fontSize:9,color:"#444",marginTop:3,letterSpacing:2}}>REPORTE FINANCIERO</div>
 </div>
 <div style={{textAlign:"right"}}>
 <div style={{fontSize:10,color:"#a78bfa",fontWeight:700}}>{rango.desde} — {rango.hasta}</div>
 <div style={{fontSize:9,color:"#333",marginTop:2}}>{new Date().toLocaleDateString("es-DO",{year:"numeric",month:"long",day:"numeric"})}</div>
 </div>
 </div>
 {/* Tarjetas resumen */}
 <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
 {[
 {l:"INGRESOS", v:fmt(res.totalV), c:"#00e676", b:"#00e67610"},
 {l:"GASTOS", v:fmt(res.totalC), c:"#ff6b35", b:"#ff6b3510"},
 {l:"GANANCIA", v:fmt(res.ganancia), c:res.ganancia>=0?"#00d4ff":"#ff3d57", b:"#00d4ff10"},
 {l:"RENTAB.", v:`${res.rentab}%`, c:res.rentab>0?"#00e676":"#ff3d57", b:"#00e67610"},
 ].map(s=>(
 <div key={s.l} style={{background:s.b,border:`1px solid ${s.c}40`,borderLeft:`3px solid ${s.c}`,borderRadius:6,padding:"12px 14px",textAlign:"center"}}>
 <div style={{fontSize:14,fontWeight:900,color:s.c}}>{s.v}</div>
 <div style={{fontSize:8,color:"#444",marginTop:4,letterSpacing:1.5}}>{s.l}</div>
 </div>
 ))}
 </div>
 {/* Gráfico de barras */}
 <div style={{background:"#0a0a0a",border:"1px solid #a78bfa20",borderRadius:6,padding:14}}>
 <div style={{fontFamily:"Orbitron,monospace",fontSize:9,color:"#a78bfa",letterSpacing:2,marginBottom:10}}> VENTAS VS GASTOS POR MES</div>
 <div style={{display:"flex",gap:4,alignItems:"flex-end",height:90}}>
 {res.mesesData.map((m,i)=>{
 const hV=res.maxBar>0?(m.ventas/res.maxBar)*80:0;
 const hC=res.maxBar>0?(m.compras/res.maxBar)*80:0;
 const gan=m.ventas-m.compras;
 return (
 <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
 {(m.ventas>0||m.compras>0)&&<div style={{fontSize:6,color:gan>=0?"#00e676":"#ff3d57",fontWeight:700}}>{gan>=0?"+":""}{Math.round(gan/1000)}K</div>}
 ...
 <span style={{fontSize:9,color:"#333",marginLeft:"auto"}}>Números = ganancia del mes en K (miles DOP)</span>
 </div>
 </div>
 ...
 {res.ventas.length===0&&<div style={{color:"#333",fontSize:11,padding:"6px 0"}}>Sin ventas en este período</div>}
 {res.ventas.map(v=>(
 ...
 <div style={{fontSize:9,color:"#333"}}>{v.codigo} · {v.fecha}</div>
 </div>
 <div style={{textAlign:"right"}}>
 ...
 {res.compras.length===0&&<div style={{color:"#333",fontSize:11,padding:"6px 0"}}>Sin compras en este período</div>}
 {res.compras.map(c=>(
 ...
 <div style={{fontSize:9,color:"#333"}}>{c.codigo} · {c.fecha}</div>
 </div>
 ...
 {/* Margen por producto */}
 <div style={{background:"#0a0a0a",border:"1px solid #00d4ff15",borderRadius:6,padding:14}}>
 <div style={{fontFamily:"Orbitron,monospace",fontSize:9,color:"#00d4ff",letterSpacing:2,marginBottom:12}}>◈ MARGEN POR PRODUCTO — TOP 8</div>
 ...
 <span style={{color:"#ccc"}}><strong style={{color:"#e0e0e0"}}>{i+1}. {p.nombre}</strong> <span style={{fontSize:10,color:"#444"}}>({p.categoria})</span></span>
 ...
 <span style={{color:"#555"}}>Gan/und: <strong style={{color:"#00e676"}}>{fmt(p.precio_venta-p.precio_compra)}</strong></span>
 <strong style={{color,fontSize:14}}>{p.mp.toFixed(1)}%</strong>
 </div>
 </div>
 <div style={{background:"#111",borderRadius:2,height:5}}>
 <div style={{width:`${Math.min(100,p.mp)}%`,height:5,borderRadius:2,background:color}} />
 </div>
 </div>
 );
 })}
 ...
 {res.deudas.map(d=>(
 <div key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #ffffff06",fontSize:11}}>
 <div>
 <div style={{color:"#ccc",fontWeight:700}}>{d.cliente_nombre}</div>
 <div style={{fontSize:9,color:"#333"}}>{d.descripcion}</div>
 </div>
 <div style={{textAlign:"right"}}>
 <div style={{color:"#ffd600",fontWeight:900}}>{fmt(d.monto-d.monto_pagado)}</div>
 <div style={{fontSize:9,color:"#444"}}>de {fmt(d.monto)}</div>
 </div>
 </div>
 ))}
 </div>
 ...
 <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
 {[
 {l:"Total ventas", v:res.ventas.length+" transacciones", c:"#00e676"},
 {l:"Total compras", v:res.compras.length+" órdenes", c:"#ff6b35"},
 {l:"Productos", v:data.productos.length+" en inventario", c:"#00d4ff"},
 {l:"Clientes", v:data.clientes.length+" registrados", c:"#00d4ff"},
 {l:"Ventas pendientes", v:fmt(res.ventas.filter(v=>v.estado==="Pendiente").reduce((s,v)=>s+v.total,0)), c:"#ffd600"},
 {l:"Deudas por cobrar", v:fmt(res.totalD), c:"#ff3d57"},
 ].map(s=>(
 <div key={s.l} style={{background:"#0a0a0a",border:"1px solid #ffffff08",borderRadius:4,padding:"8px 12px",display:"flex",justifyContent:"space-between",fontSize:11}}>
 <span style={{color:"#444"}}>{s.l}</span>
 <strong style={{color:s.c}}>{s.v}</strong>
 </div>
 ))}
 </div>
 {/* Footer */}
 <div style={{paddingTop:10,borderTop:"1px solid #ffffff08",display:"flex",justifyContent:"space-between",fontSize:9,color:"#333"}}>
